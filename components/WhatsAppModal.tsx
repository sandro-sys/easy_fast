...
export function WhatsAppModal({ data, onClose }: WhatsAppModalProps) {
  const phone = data.guest_phone.replace(/\D/g, "");
  const text = encodeURIComponent(
    `Olá ${data.guest_name}! Sua reserva foi confirmada para o dia ${data.reservation_date} às ${data.reservation_time}.${data.observation ? ` Observação: ${data.observation}` : ""} Obrigado!`
  );
