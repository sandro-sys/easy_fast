...
export function Header({ userEmail, isMaster, companyName }: HeaderProps) {
  return (
    <header className="sticky top-0 z-10 border-b border-white/10 bg-[#171A1E] shadow-lg shadow-black/20">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
        <Link href="/" className="flex flex-col">
          <span className="text-xl font-bold text-white drop-shadow-[0_0_12px_rgba(16,185,129,0.5)]">{APP_NAME}</span>
          <span className="text-xs text-emerald-400/90 drop-shadow-[0_0_8px_rgba(16,185,129,0.35)]">{APP_TAGLINE}</span>
        </Link>
        <nav className="flex items-center gap-4">
          {userEmail ? (
            ...
            <>
              <Link href="/planos" className="text-sm font-medium text-slate-300 hover:text-[#32C76A] transition-colors">
                Planos
              </Link>
